...
class DuplicateVC: UIViewController {
           ...
           // self.txtView.textColor = UIColor.hexStringToUIColor(hex: "#E4E4E4")
            if (MyDefaults().language ?? "") as String ==  "en"{
               // self.txtView.textColor = UIColor.hexStringToUIColor(hex: "#C0C8DE")
                ...
                print("Descriptions".LocalizableString(localization: "da"))
            } else{
               // self.txtView.textColor = UIColor.hexStringToUIColor(hex: "#C0C8DE")
...
//        }
    ...
    func changeLanguage(strLanguage:String) {
        ...
        self.lblTitleNav.text = "Duplicate_Auction".LocalizableString(localization: strLanguage)
...
//self.txtCondition.placeholder = "Choose".LocalizableString(localization: strLanguage)
    ...
    func callServiceSubmitAPI() {
        ...
        var cameraImages = [[String:Any]]()
        for items in self.imageArray {
            
            if items["isselected"] as! String == "yes" {
                if items["type"] as! String == "url" {
                    let myStringArr = items["url"] as! String
                    let theURL = URL(string: myStringArr)  //use your URL
                    let fileNameWithExt = theURL?.lastPathComponent
                    serverImages.append(fileNameWithExt!)
                } else {
                    var dict = [String:Any]()
                    dict ["uploadfile"] = "product_image[]"
                    dict ["type"] = "image"
                    dict ["png"] = items["png"] as! UIImage
                    dict["isselected"] = "yes"
                    cameraImages.append(dict)
                    print(self.imageArray)
        ...
        print(serverImages.joined(separator: ","))
        ...
        let parameter: [String: Any] = ["language": MyDefaults().language ?? AnyObject.self,
                                         ...
                                         "old_product_image":serverImages.joined(separator: ",")]
        print(parameter)
        HTTPService.uploadimage(url: getPosProductAPI, imageToUpload: cameraImages, authtoken: "", showHud: true, text: loading, VC: self, parameters: parameter) { (response) in
        debugPrint(response)
                    debugPrint(response)
                  //  HideHud()
                    if response.count != nil {
                        ...
                        if status == 200  {
                          ...
                          //  self.navigationController?.backToViewController(viewController: DuplicateVC.self)
                            // self.showErrorPopup(message: message, title: ALERTMESSAGE)
    ...
    @IBAction func actionOnSameRegisterAddress(_ Sender:Any){
        ...
        if  objActiveAuction.address.isEmpty {
            self.lblAddress.text = objActiveAuction.address
        } else {
            self.lblAddress.text = objActiveAuction.address
        }
...
extension DuplicateVC: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
            if self.collectionProductImage == collectionView {
                let cell: AddPostDuplicateImageCollectionViewCell = collectionView.dequeueReusableCell(withReuseIdentifier: "AddPostDuplicateImageCollectionViewCell", for: indexPath) as! AddPostDuplicateImageCollectionViewCell
...
extension DuplicateVC: UIImagePickerControllerDelegate, UINavigationControllerDelegate,getSelectAddtPostPopUpSubCategory,getSelectAddPostEditProperty {
...
extension DuplicateVC:UITextFieldDelegate {
//    func textViewDidBeginEditing(_ textView: UITextView) {
//        if textView.textColor == UIColor.hexStringToUIColor(hex: "#C0C8DE") {
//            textView.text = nil
//            if (MyDefaults().language ?? "") as String ==  "en"{
//                textView.text = "Descriptions".LocalizableString(localization: "en")
//
//            } else {
//                textView.text = "Descriptions".LocalizableString(localization: "da")
//            }
//
...
extension  DuplicateVC:UITableViewDelegate,UITableViewDataSource{
...
//}
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            var cell : AddPostDuplicateTableViewCell?
                 let dict = arraypropertry[indexPath.row]
                 cell = tableView.dequeueReusableCell(withIdentifier: "AddPostDuplicateTableViewCell", for: indexPath) as? AddPostDuplicateTableViewCell
    ...
    extension DuplicateVC: GMSAutocompleteViewControllerDelegate {
    ...
    class AddPostDuplicateTableViewCell: UITableViewCell {
        @IBOutlet weak var lblValue : UILabel!
        @IBOutlet weak var lblName : UILabel!
        @IBOutlet weak var btnSubCategory : UIButton!
    }
class AddPostDuplicateImageCollectionViewCell: UICollectionViewCell {
    @IBOutlet weak var btnCross : UIButton!
    @IBOutlet weak var btnUpload : UIButton!
    @IBOutlet weak var imgView : UIImageView!
    func configureCellImageCategory(cell:AddPostDuplicateImageCollectionViewCell,image:String)  {
